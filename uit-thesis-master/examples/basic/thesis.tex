...
%Figure \ref{fig:time_chart500} shows the result. We can see that the comparison between images and annotations is the part of the system that takes the longest time followed by the part that finds folders and read metadata from the raw data. This is because of the of the high number of data to process.

%Figure \ref{fig:time_chart500} shows the time to execute different task in the system. We can see that the comparison between images and annotations is the part of the system that takes the longest time followed by the part that finds folders and read metadata from the raw data. This is because of the of the high number of data to process.
...
%This is because there are more data to compare and will so 
...
\textbf{bruker mye cpu i starten og midten, men roer seg ned etter hvert - ligger da på rundt 25\% CPU. På starten driver programmet med traversering og lesing av metadata, så excel-lesing og så comparing. Comparing kan bruke mye cpu på starten hvis det må til disk å lese, men etter hvert ligger kanskje mye i memory/cachen..?}

We measured the CPU utilities to \textbf{see/study/examine} the CPU percentage of the system. As we can see in Figure \ref{fig:chart_cpu}, 
...
\textbf{Kan se at den i starten bruker litt minne, en peak på 94\% før den daler ned til rundt 90\% før den igjen stiger til rundt 94\% og blir i dette området helt til programmet er ferdig...}

We measured the memory utilities to \textbf{see/study/examine} the memory usage of the system. As we can see in Figure \ref{fig:chart_memory},
...
We measured the disk utilities to \textbf{see/study/examine} the disk I/O statistics of the system. As we can see in Figure \ref{fig:chart_read_count} and Figure \ref{fig:chart_read_count_since_last}...

\iffalse
\begin{figure}
\centering
\includegraphics[width=\textwidth]{chart_read_count.png}
\caption{Figure showing disk reads...}
\label{fig:chart_read_count}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\textwidth]{chart_read_count_since_last.png}
\caption{Figure showing disk read compared to last disk read...}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{chart_write_count.png}
\caption{Figure showing disk writes...}
\label{fig:chart_write_count}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\textwidth]{chart_write_count_since_last.png}
\caption{Figure showing disk write compared to last disk write...}
...
\begin{itemize}
%\item CONCURRENT
%\item IMPLEMENT VS AS ABSTRACTIONS (CLASS) AND SUBPROCESSES
%\item QUERY: COMBINE DATA FROM MULTIPLE SENSORS
%\item UPDATE DATASTORAGE FREQUENT, NOT ON-DEMAND

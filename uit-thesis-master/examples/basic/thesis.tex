...
\textit{Drawback with Python - not that good at parallel/concurrency..}
...
\textbf{libraries used in this implementation:}
...
\subsection{Data Storage} \label{ssec:storage_fused}
The data storage contains folders with labeled and sorted animals, but these images consists of no metadata so we don't have any information of when the image is taken. Therefore, the solution is to traverse the whole data storage and find the original images with metadata and compare these to the excel-files containing information like date, time, location and site.
\textbf{HOW DID I TRAVERSE}

%\subsection{Exchangeable Image File Format - EXIF}
%To read the metadata, as mentioned in Section \ref{sec:des_fused}, we used a Python module called  EXIF 2.1.2\footnote{\url{https://pypi.python.org/pypi/ExifRead}}, to extract the metadata from the jpg-files.

%\subsection{Pandas} %moved to overview.
%Pandas\footnote{\url{http://pandas.pydata.org/}} is an open source Python Data Analysis Library which provides high-performance and easy-to-use data structures. It is a NUMFocus\footnote{\url{https://www.numfocus.org/open-source-projects/}} sponsored project which will help ensure success of the development of pandas as a open-source project in world class. A dataframe\footnote{\url{https://pandas.pydata.org/pandas-docs/stable/generated/pandas.DataFrame.html}} is a two-dimensional, dynamic tabular data structure with labeled axes (rows and columns), just like an excel-files.
...
\textbf{HVORDAN EXTRACTED JEG DATA FRA EN DATAFRAME?!}
\textit{We use Pandas dataframe, described in Section \ref{sssec:imp_df}, to extract relevant data from the excel-files in the data storage.}
...
\textit{Excel - pandas dataframe, lese ut information. Lagre i en string med info som datetime, location, site, year, animal og antall animals.}
...
\textbf{Valgte å fokusere på 3 dyr ettersom de var de som det var flest av i excel-arkene.. and time constraints.. ville ha noe å vise enn ingenting}

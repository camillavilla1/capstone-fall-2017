...
We decided to measure execution time on the five different experiments. We measured execution time on the new csv-file that contained only fused data from Varanger and the new CSV-file that contained fused data from Varanger and Nordkynn. We also tried to run three experiments where we tried to run the comparing concurrently with 4, 100 and 500 processes by using a multiprocessing library in Python that supports spawning processes using an API similar to the threading module.
...
\textbf{Use long time. Have 4 figures showing execution time in minutes.. Figures \ref{fig:time_chart}, \ref{fig:time_chart2}, \ref{fig:time_chart3}, \ref{fig:time_chart500}, \ref{fig:time_chart500new}. Most time during finding folder and read metadata and comparing.. Read excel file is faaaast!!}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{chart_500proc.png}
\caption{Figure showing time to execute the system (in minutes).}

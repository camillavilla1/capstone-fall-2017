...
The excel-sheets contains from 17 000 to 70 000 rows with information about each picture taken. \textit{ Eller "thousands of rows with data?" Må få frem at det er masse data som ligger i excel-ark?}
...
%However, the main components in the system are the data storage, the fused data, the virtual sensors and the user.
The architecture of the system is presented in Figure \ref{fig:architecture}. \textbf{The arrows indicates the communication lines.... eller flyten av data eller hvem som er aktiv i å gjøre akses/sende - NOE SÅNT HER OGSÅ I NY FIGUR}
...
The data also contains metadata about each image.(\textbf{More on the metadata!) --- Referere til chap. om the data set?}

\subsection{The Data Set}
Hvordan de er samlet, hvordan de er organisert
NEVNES OGSÅ I DESIGN, HA ET EGET KAPITTEL OM DETTE FØR ARCH OG DESIGN FOR Å SAMLE DISSE 2?

\ OLD 
\textit{The physical sensors transmit their data to the data storage, so the data storage consists of several set of images from the different bait-camera sensors. These pictures are organized by the year they where taken and also where the camera was placed in the Arctic Tundra in Finnmark, Norway. 
...
\textit{John M.: Kand u ha flre identeiske virtuelel sensorer for å prosesere i parelll? Og Er det sensorene som visualiserer, eller returnerer de data som kan  visualiseres av klienten= ut fra 4.3 høres det ut som siste}
